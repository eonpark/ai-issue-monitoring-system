...
공개 검색 API와 LLM을 활용해 국내외 주요 이슈를 수집, 분석, 검증하고 slack 및 웹 Admin 페이지로 보고하는 로컬 실행용 AI 에이전트
...
발표용 README는 전체 흐름을 중심으로 설명한다.
세부 명세는 아래 문서로 분리했다.

- `agents.md`
  - Collector, Analyzer, Validator, Formatter, Publisher의 역할과 입출력을 정리한 문서
  - Router의 deterministic state routing과 failure fallback 정책 설명

- `skills.md`
  - Tavily Search, LLM Analyze, Slack Send 등 외부 API/모델 호출 단위의 Skill 명세 문서
  - Collector query 설정과 Analyzer 판단 기준 reference 포함

- `evaluation_metrics.md`
  - 시스템 평가 기준과 성능 지표 정의 문서
  - 수집 균형, 주요 이슈 비율, 원문 감사 결과, 중복 제거율, 최종 OK 건수 등을 평가 기준으로 정리
